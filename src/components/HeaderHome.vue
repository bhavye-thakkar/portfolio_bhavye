...
/**
 * While the Experience story page is open the home page is `position: fixed`,
 * so these triggers are measuring nothing and would report whatever the stale
 * geometry says. Experience is the honest answer for the whole of that page.
 */
const setActive = (section: ActiveLink | null) => {
  if (storyActive.value) return;
  activeLink.value = section;
  updateBarPosition();
};

watch(storyActive, (active) => {
  if (!active) return;
  activeLink.value = "experience";
  updateBarPosition();
});
