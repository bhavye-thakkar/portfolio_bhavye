...
import { figureShadow } from "./figure-shadow";
...
const tick = () => {
  const onStage = sceneWeights.contact > 0.001;
  group.visible = onStage;
  figureShadow.tick();
  // the goodbye's canvas fade belongs to this scene only
  if (!onStage) goodbye.clearFade();
};

const destroy = () => {
  gsap.ticker.remove(tick);
  shadow.destroy();
  figureShadow.destroy();
};
