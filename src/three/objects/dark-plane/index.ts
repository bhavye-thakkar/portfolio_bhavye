...
const tick = () => {
  ...
  // The Experience scene inherits this window on the grid floor, so it holds
  // the plane open past the point where About alone would close it.
  const onExperienceStage = sceneWeights.experience > 0.001;

  if (
    !onExperienceStage &&
    (progress.in < 0.001 || (progress.in === 1 && progress.out >= 0.999) || progress.out === 1)
  ) {
    mesh.visible = false;
    return;
  } else {
