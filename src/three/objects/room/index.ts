...
/**
 * The CV document, on the free desk in front of the penguin.
 *
 ...
 * page, and the same prop, the same cyan tab and the same panel. The two are
 ...
 * page scrolls.
 *
 * Full size, the same as the Experience one. The envelope this replaced was
 * scaled to 0.85 here because an envelope the size of a mouse mat looked
 * wrong; a CV is an A4 sheet, and A4 next to a mouse is exactly the right
 * size. From the hero camera the flat sheet is a strip a few dozen pixels
 * wide, so every bit of size goes toward it reading as a document.
 */
...
const init = () => {
  ...
  // ponytail: no Timex on this desk any more (owner, 2026-09-09). He wears it
  // in Experience, see `avatar/watch.ts`; the desk prop and its object page
  // went with it.
};
